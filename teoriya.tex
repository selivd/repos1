...
\subsection{Построение множества через функцию цены}
Мы будем искать множество достижимости с использованием функции цены, значениями которой будет расстояние выбранной точки $x(t)$ от искомого множества $\XX[t,t_0,\XX_0]$. Согласно \defref{def:XX2} нам необходимо найти наиболее подходящее управление, которое позволит сократить до мининмума это расстояние при любой возможной реализации помехи. Для этого используется минимаксный подход, описанный в \mycite{ABK_upr_i_nabl}.
...
Поскольку значения фунции ищутся через вычисление $\min,\max$ по пространству функций, то построить в явном виде эту фунцию представляется затруднительным.
