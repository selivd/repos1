...
$$
\int\limits_{t_0}^t d\phi(w) \int\limits_t^w ds(\cdot) =\int\limits_t^{t_0} ds \int\limits_{t_0}^s d\phi(w) (\cdot),
$$
$$
\int\limits_{t_0}^t d\phi(w) \int\limits_w^{t_0} ds(\cdot) =\int\limits_t^{t_0} ds \int\limits_{s}^t d\phi(w) (\cdot),
$$
и делая замену переменных
$$
\begin{aligned}
S(t_0,t_1)&=\intd{t_0}{t_1}\tilde{S}(w,t_1)\,d\phi(w)=\int\limits^{t_0}_{t_1}\left\{  G^T_2(w,t)G^T_1(t_0,w)\ell(w) + \mu(w) G_2^T(w,t)c \right\}\,d\phi(w), \\
