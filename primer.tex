...
Поскольку мы рассматриваем задачу в классе программных управлений, мы не можем строить управление в зависимости от текущего состояния системы, а только заранее  его определять. Поэтому, в формуле для $V(t,x)$ нельзя проводить оптимизацию отдельно для ''до'' и ''после'', так как момент переключения $\tau$ не известен заранее.
$$
V(t,x)=\min_{u_1,u_2\in\mathcal{P}}\max_{v_1,v_2\in\mathcal{W}}\max_{\tau}\left\{ d^2(x_0{\mid_{x(t)=x}},\mathcal{X}_0) + \left(\ts{x(\tau),c}-\gamma\right)^2 \right\}.
...
$$
V(t,x)=\min_{u_1,u_2 \in\mathcal{P}}\max_{v_1,v_2\in\mathcal{W}}\max_{\tau}\left\{
...
$$
Легко видеть, что $T(\tau,v(s))$, являясь линейным по $v$, не является таковым по $\tau$. Тогда вместо $\tau$ возьмем функцию  $\tau(w)=\phi(w)$ и преобразуем 
$$
...
$$
\begin{gathered}
 V(t,x)=\min_{u_1,u_2}\max_{v_1,v_2}\min_{\phi(w)} \max_{\ell(w)}\max_{\mu(w)}\intd{t_0}{t} \phi(w)\bigg\{  \\
...
$$
\begin{aligned}
S(t_0,t_1)&=\intd{t_0}{t_1}\phi(w)\tilde{S}(w,t_1)\,dw=\int\limits^{t_0}_{t_1}\phi(w)\left\{  G^T_1(t_0,w)G^T_2(w,t)\ell(w) + \mu(w) G_2^T(w,t)c \right\}\,dw, \\
S_1(t_0,t_1)&=\intd{t_0}{t_1}\phi(w)\tilde{S}_1(t_0,w)\,dw,=\intd{t_0}{t_1}\phi(w)\left\{ G^T_1(t_0,w)  \ell(w) \right\}\,dw,\\
K(\ell,\mu,\mathcal{X}_0)&=\intd{t_0}{t} \phi(w)\left[\frac{\mu(w)^2}{4} + \rho(\ell(w)\mid\mathcal{X}_0) + \frac{\|\ell(w)\|^2}{4} \right]\,dw
\end{aligned}
$$
придем к 
\begin{equation}
...
\begin{gathered}
 V(t,x)=\min_{u_1,u_2}\min_{\phi} \max_{\ell}\max_{\mu} \bigg\{  \\
  S^T(t_0,t)x + \intd{t}{t_0} S^T(t_0,s)B_2(s)u_2(s)\,ds + \intd{t}{t_0}\rho(S^T(t_0,s)\mid C_2(s)\mathcal{W}_2(s)) \,ds \\
 + \intd{t}{t_0} S^T_1(s,t)B_1(s)u_1(s)+ \intd{t}{t_0}\rho(S^T_1(s,t)\mid C_1(s)\mathcal{W}_1)\,ds - \\
 - K(\ell,\mu,\mathcal{X}_0)  \bigg\}.
\end{gathered}
$$
Далее, мы хотим поменять $\min_{u_1,u_2}(\cdot)$ на опорную функцию, но полученное выше выражение уже не является вогнутым по $\ell,\mu$. Поэтому, мы прибегаем к овыпуклению нужных членов и наконец получаем 

$$
\begin{gathered}
\label{eq:V-fin}
 V(t,x)=\min_{\phi} \max_{\ell}\max_{\mu} \bigg\{  \\
  S^T(t_0,t)x - \intd{t_0}{t} \rho(S_1(s,t)\mid B_1(s)\mathcal{P}_1(s))\,ds - \intd{t_0}{t} \rho(S(t_0,s)\mid B_2(s)\mathcal{P}_2(s))\,ds - \\ - \mbox{conv}\left\{ -\intd{t}{t_0}\rho(S_1(s,t)\mid C_1(s)\mathcal{W}_1(s))\,ds-\intd{t}{t_0}\rho(S(t_0,s)\mid C_2(s)\mathcal{W}_2(s)) \,ds + K(\ell,\mu,\mathcal{X}_0)  \right\} \bigg\}.
\end{gathered}
$$



\subsection{Проблема явного выражения $\ell(w),\mu(w)$}

Выражение стоящее под знаком $\min\max$ в \eqref{eq:V-fin} является линейной функцией по $\tau(w)$  \cmnt{(лемма о интегрировании овыпукления)}


\cmnt{Я пытаюсь взять производную по $\mu$}

Произодная от опорной функции элипсоида 
$$
\frac{d}{d\mu}\rho(\ell(\mu)\mid\Ell{q,Q})=\frac{d}{d\ell}(\ts{\ell,q}+\ts{\ell,Q\ell}^\frac12)\frac{d\ell}{d\mu}=\left( q+\frac{\ell^TQ}{\ts{\ell,Q\ell}^{\frac12}}\right)\frac{d\ell}{d\mu}
$$

Для краткости обозначим $\Ell{\tilde{q},\tilde{Q}}=\Ell{B_2q,B^T_2QB_2}=B_2\mathcal{P}_2)$, тогда 
\begin{align}
	\frac{d}{d\mu}\intd{t_0}{t} \rho(S(t_0,s)\mid B_2(s)\mathcal{P}_2(s))\,ds = \\
	\intd{t_0}{t}\left(\tilde{q}+\frac{S(t_0,s)Q^T}{\ts{S(t_0,s),\tilde{Q}S(t_0,s)}^{\frac12}}\right)\, \cdot \left(\intd {t_0} s \phi(w)G^T_2c(w,t)dw\right)ds
\end{align}

Затем нужно как-то взять производную по $\mu$ для $\mbox{conv}(\cdot)$. Я пока затрудняюсь это сделать в $\mathbb{R}^n$. И здесь, наверное, как раз можно сначала посчитать $\mbox{conv}(\cdot)$ конкретно для примера. Но я пока здесь остановился. Производную беру, чтобы на основании выпуклости (линейности) найти единственный минимум. 
